...
def test(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = Path(increment_path(Path(opt.data_dir) / 'runs' / 'test' /(opt.name+f'({save_name_add})'), exist_ok=opt.exist_ok))  # increment run
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=32, help='size of each image batch')
