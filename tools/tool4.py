...
def func_get_zhuan(src):
    ...
    ret, binPic = cv2.threshold(grayPic, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # ret, binPic = cv2.threshold(grayPic, 130, 255, cv2.THRESH_BINARY)
    print(binPic.shape)
    ...
    # 找出轮廓
    _, contours, hierarchy = cv2.findContours(cannyPic, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
...
src_label_imdir = os.path.join(data_dir, 'tile_round1_train_20201231/train_imgs')
ims_l = os.listdir(src_label_imdir)
for name in ims_l[:1]:
    image_file = os.path.join(src_label_imdir, name)
