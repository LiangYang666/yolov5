...
def test(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = Path(increment_path(Path(opt.data_dir) / 'runs' / 'test' /(opt.name+f'_{save_name_add}_'), exist_ok=opt.exist_ok))  # increment run
    ...
    if not training:        # 如果不是在训练的时候调用
        data['val'] = os.path.join(opt.data_dir, opt.test_txt)
    ...
    if not training:
        ...
        dataloader = create_dataloader(path, imgsz, batch_size, model.stride.max(), opt, pad=0, rect=True)[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='runs/train/exp(allzx0)(OK 600 yolov5s)/weights/ckp_allzx0_4c_epoch600.pt', help='model.pt path(s)')
    ...
    parser.add_argument('-test', '-test_txtl_file', dest='test_txt',
                        type=str, default='testzx0.txt', help="test txt file")
    ...
    print(opt)
    # print(opt.weights)
    # python test.py -b 6 --data data/bydata.yaml --weights checkpoints/ckp_allby0_2c_epoch600.pt  --device 1 -test testby0.txt
